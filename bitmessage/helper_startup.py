...
storeConfigFilesInSameDirectoryAsProgramByDefault = True# The user may de-select Portable Mode in the settings if they want the config files to stay in the application data folder.
...
def loadConfig():
    ...
    if needToCreateKeysFile:
        ...
        shared.config.set('bitmessagesettings', 'identiconsuffix', ''.join(random.choice("123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz") for x in xrange(12))) # a twelve character pseudo-password to salt the identicons
	...
	#shared.config.set(
        ...
        #shared.config.set(
        #    'bitmessagesettings', 'apiinterface', '127.0.0.1')
        #shared.config.set(
...
def loadPubkeys():
    # Load pubkeys from pubkeys.dat
    conn = sqlite3.connect('pubkeys.dat')
    conn.text_factory=str
    cur = conn.cursor()
    try:
        cur.execute("SELECT address,addressversion,transmitdata,time,usedpersonally FROM pubkeys")
        query = cur.fetchall()
        for row in query:
            shared.hadPubkeys[row[0]] = row
        cur.close()
        conn.close()
    except Exception as e:
        if 'no such table' in str(e):
            cur.execute("CREATE TABLE pubkeys (address text, addressversion int, transmitdata blob, time int, usedpersonally text, UNIQUE(address) ON CONFLICT REPLACE)")
            conn.commit()
            cur.close()
            conn.close()
            print "No pubkey,store database initialized."
        else:
            print e
